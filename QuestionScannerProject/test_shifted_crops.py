...
# OCR helper for digit array using clean dynamic stitching and fallback
def ocr_digits(boxes):
    ...
    # Stitch boxes into a single horizontal word image to boost CRAFT detector accuracy
    ...
    res = reader.readtext(combined, allowlist='0123456789', detail=0)
    text = "".join(res).replace(" ", "")
    
    if len(text) == len(boxes):
        return text
        
    # Box-by-box fallback with side padding
    result = []
...
def ocr_answers(boxes):
    ...
    res = reader.readtext(combined, allowlist='abcdABCD', detail=0)
    ...
    for idx, img_box in enumerate(processed):
        if is_box_blank(boxes[idx]):
            ...
            padded = cv2.copyMakeBorder(img_box, 20, 20, 40, 40, cv2.BORDER_CONSTANT, value=[255, 255, 255])
            res_ind = reader.readtext(padded, allowlist='abcdABCD', detail=0)
...
if not is_box_blank(class_box):
    ...
    class_res = reader.readtext(padded_class, allowlist='0123456789', detail=0)
...
if not is_box_blank(sec_box):
    ...
    sec_res = reader.readtext(padded_sec, allowlist='abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ', detail=0)
    sec_text = "".join(sec_res).replace(" ", "").upper()

answers = ocr_answers(ans_boxes)

print("--- Shifted OCR Results ---")
